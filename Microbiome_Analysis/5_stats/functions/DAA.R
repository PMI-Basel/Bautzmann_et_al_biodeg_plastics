...
run_all_DAA <- run_all_DAA <- function(feature_table, meta_data, group, DAT_sample_names, reference){
  ...
  meta_data[,group] <- as.character(meta_data[,group])
  #aldex2
  ...
  summ$score <- rowSums(summ[,2:4])
  return(list(summ, aldex2_sign, maaslin2_sign, ancom_sign, metagenomeSeq_sign))
}
#
